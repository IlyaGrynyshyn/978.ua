...
def product_spec(category):
    ...
    search_filter_body = """<div class="aside">{}</div>"""
    mid_res = ""
    for (feature_name, feature_filter_name), feature_values in feature_and_values.items():
        feature_name_html = f"""<h3 class="aside-title">{feature_name}</h3>"""
        feature_values_res = ""
        for f_v in feature_values:
            mid_feature_values_res = \
                "<div class='checkbox-filter'><div class=''><input type='checkbox' name='{f_f_name}' " \
                "value='{feature_name}'> <label> {feature_name} </label></div></div>".format(
